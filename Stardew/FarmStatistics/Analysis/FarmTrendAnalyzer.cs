...
namespace FarmStatistics.Analysis
{
    ...
    public class FarmTrendAnalyzer : BaseAnalysisProvider<TrendAnalysisResult>
    {
        ...
        public async Task<DailyFarmData> GenerateCurrentDailyData()
        {
            try
            {
                var farmData = await _dataCollector.CollectCurrentDataOptimizedAsync();
                ...
                return new DailyFarmData
                {
                    ...
                    TotalEarnings = ParseEarnings(farmData.OverviewData?.TotalEarnings ?? "0G"),
                    CropCount = farmData.CropStatistics?.Count ?? 0,
                    AnimalCount = farmData.AnimalStatistics?.Count ?? 0,
                    TotalCropsHarvested = ParseCount(farmData.OverviewData?.TotalCropsHarvested ?? "0개"),
                    TotalAnimalProducts = ParseCount(farmData.OverviewData?.TotalAnimalProducts ?? "0개"),
        ...
        private async Task<TrendAnalysisResult> AnalyzeProfitTrend(AnalysisParameters parameters)
        {
            ...
            try
            {
                ...
                var dailyData = GetHistoricalData(days);
        ...
        private async Task<TrendAnalysisResult> AnalyzeProductionTrend(AnalysisParameters parameters)
        {
            ...
            try
            {
                ...
                var productions = dailyData.Select(d => d.TotalCropsHarvested + d.TotalAnimalProducts).ToList();
        ...
        /// 히스토리컬 데이터를 가져옵니다.
        /// </summary>
        private List<DailyFarmData> GetHistoricalData(int days)
        {
            lock (_dataLock)
            {
                ...
                return _historicalData.Values
                    ...
                    .Where(data => data.Date >= cutoffDate)
        ...
        {
            if (values.Count < 2)
                return new LinearTrend { Slope = 0, Intercept = 0, RSquared = 0 };
        ...
        private TrendAnalysisResult CreateInsufficientDataResult(string analysisType)
        {
            return new TrendAnalysisResult
            {
                ...
                Summary = "분석을 위한 충분한 데이터가 없습니다. 더 많은 게임 플레이 후 다시 시도해주세요.",
                Recommendations = new List<string> { "더 많은 데이터를 수집하기 위해 게임을 더 플레이해주세요." }
            };
        }
