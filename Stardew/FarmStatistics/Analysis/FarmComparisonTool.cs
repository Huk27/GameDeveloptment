...
namespace FarmStatistics.Analysis
{
    ...
    public class FarmComparisonTool : BaseAnalysisProvider<ComparisonResult>
    {
        ...
        public async Task<FarmData> GetCurrentFarmData()
        {
            try
            {
                var farmStats = await _dataCollector.CollectCurrentDataOptimizedAsync();
                
                return new FarmData
                {
                    Name = "현재 농장",
                    TotalEarnings = ParseEarnings(farmStats.OverviewData?.TotalEarnings ?? "0G"),
                    CropTypes = farmStats.CropStatistics?.Count ?? 0,
                    AnimalTypes = farmStats.AnimalStatistics?.Count ?? 0,
                    TotalCropsHarvested = ParseCount(farmStats.OverviewData?.TotalCropsHarvested ?? "0개"),
                    TotalAnimalProducts = ParseCount(farmStats.OverviewData?.TotalAnimalProducts ?? "0개"),
