        ...
        public async Task<AnalysisDashboard> GenerateDashboardAsync()
        {
            try
            {
                ...
                var analysisTask = Task.WhenAll(
                    AnalyzeTrendAsync("profit_trend", analysisParameters),
                    AnalyzeTrendAsync("production_trend", analysisParameters),
                    AnalyzeTrendAsync("efficiency_trend", analysisParameters),
                    AnalyzeComparisonAsync("comprehensive_comparison", analysisParameters)
                );
                
                var results = await analysisTask;
                var profitTrend = results[0];
                var productionTrend = results[1];
                var efficiencyTrend = results[2];
                var comprehensiveComparison = results[3] as ComparisonResult;
